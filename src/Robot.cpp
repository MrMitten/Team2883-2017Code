...
private:
	...
	void HoppaDrop()
	{
		...
		if(timer.Get() < 2.5 && timer.Get() > 1.125)
			{
			server.SetSource(cam2);
			move = 0;
			rotation = 0;
		...
		if(timer.Get() > 2.75 && timer.Get() < 3.35)
			{
			server.SetSource(cam1);
		...
		if(timer.Get() > 3.35 && timer.Get() < 4.25)
			{
			hoppercycle.Set(0);
			hooper.Set(0);
				server.SetSource(cam1);
				...
				if(move != 0)
				{
				myRobot.ArcadeDrive(move*1.8, rotation*.8);
				hoppercycle.Set(-1);
				hooper.Set(-1);
				}
				else
			...
			if(timer.Get() > 4.25 && timer.Get() < 10)
				{
				if((shot = true))
				{
					server.SetSource(cam1);
					...
					shooter.Set((motorspeed-.025)/(power.GetVoltage()/12));
				}
				}
	...
	void UpdateControlsMotors()
	{
		...
		if(motorswitch)
		{
			shooter.Set((motorspeed - .055)/(power.GetVoltage()/12));
			//shooter.Set(1);
		}
		else
		{
	...
	void UpdateVision()
	{
		std::cout << "Center Left Right";
			...
			for(unsigned int i = 0; i < arr.size(); i++)
			{
				...
				{
					//myRobot.Drive(0.15*((arr[0] - 80)/80) + .05,-1.0);
					rotation = +0.32*((arr[0] - 103)/57) + .6;
					std::cout << "right";
				}
				else if(arr[0] < 102)
				{
					//myRobot.Drive(0.15*((80 - arr[0])/80) +.05, 1.0);
					rotation = -0.32*((103 - arr[0])/103) -.6;
					std::cout << "left";
				}
				}
			...
			std::cout << "Center Up Down";
				for(unsigned int i = 0; i < ary.size(); i++)
				{
						std::cout << ary[0] << "  ";
								...
								{
									//myRobot.Drive(0.35*((ary[0] - 40)/120) + .15,0);
									move = -0.55*((ary[0] - 20)/140) - .3;
									//std::cout << move;
								}
								else if(ary[0] < 19)
								{
									//myRobot.Drive(-0.35*((40 - ary[0])/40) -.15, 0);
									move = +0.25*((20 - ary[0])/20) +.3;
									//std::cout << move;
								}
								}
								else
					...
					if((ary[0] - 20) < 1 && (arr[0] - 103) < 3 && (arr[0] - 103) > -3)
								{
										hoppercycle.Set(-1);
										hooper.Set(-1);
										shot = true;
